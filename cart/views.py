...
# Context dictionary: Sends data
# (cart object and cart items) from the view to the template
    context = {
     'cart': cart,
...
# to enable the use to add and remove items from their cart

@login_required
def add_to_cart(request, product_id):
    """ Add a product and increase the quantity
    which will be reflected in the shopping bag """

    # Get object (product) from database,
    # if object doesn't exist then display 404 error page
    ...
    if not created:
        ...
        # cart then increase the quantity to
...
def adjust_cart(request,
                item_id):
    """ Adjust the quantity of a cart
    item (database-based, replaces
    session code) """
    ...
    # f" Updated {'product':product.name} to your cart!")
